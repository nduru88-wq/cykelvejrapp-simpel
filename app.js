...
function renderWeekSummary(){
  ...
  for(let i=0;i<5;i++){
    ...
    btn.className=`week-day ${data.label.cls}${selected?" selected":""}`;
    btn.innerHTML=`
      <span class="week-day-name">${capitalize(date.toLocaleDateString("da-DK",{weekday:"short"}))} ${date.getDate()}/${date.getMonth()+1}</span>
...
function renderMode(){
  document.getElementById("dailySummary").classList.toggle("hidden",weekMode);
  document.getElementById("weekSummary").classList.toggle("hidden",!weekMode);
